insert into  TOTAL_VIP_month_0509
  ...
  select b.game_ab,
    ...
    from (select game_au,
           ...
           where logtime = to_char£¨sysdate - 2£©) a
    ...
    left join (select userid, sum(cash_need) as consumption_7
                 from bitask.t_dw_cb_glog_shoptrade
                where logtime >= sysdate - 7
                group by userid) i
    ...
    left join (select groupname,
                 ...
                 from bitask.t_dw_cb_dic_serverlist) k
    ...
    left join (select c.userid,
                 ...
                 from (select a.userid, b.f_city_name, count(*) as cnt
                         from (select userid, ip_num
                                 from bitask.t_dw_cb_glog_accountlogout
                                where  logtime >= trunc(sysdate - 15, 'MM') - 1
    ...
    left join bitask.t_dw_cb_account_status m
