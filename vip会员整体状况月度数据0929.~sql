  ...
  for i in 1 .. 16 loop
    insert into vip_status0919
      select last_day(add_months(sysdate, -i)) as time,
        ...
        from (select t.game_au, t.userid, t.passport, t.rank
                from bitask.t_dw_vip_vipinfo t
               where t.logtime = trunc(last_day(add_months(sysdate, -i)))) a
        ...
        left join (select t.game_au, t.userid, t.passport, t.rank
                     from bitask.t_dw_vip_vipinfo t
                    where t.logtime =
                          trunc(last_day(add_months(sysdate, - (i + 1))))) c
        ...
        left join (select userid, sum(cash_need) / 100 as consume
