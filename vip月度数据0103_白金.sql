...
from 
(select month ,
       sum(case when rank = 3 then 1 else 0 end) as vip_num,
       ...
       sum(case when rank = 3 and (rank_lm < 3 or rank_lm is null) then 1 else 0 end) as vip_add,
       sum(case when rank < 3 and rank_lm = 3 then 1 else 0 end）as vip_lost
