insert into TOTAL_VIP_weekly_0422
  select b.game_ab,
         a.game_au,
         a.userid,
         a.passport,
         ...
         --e.pay2016,
         f.login_cnt_7,
         trunc(f.login_time_7, 1) as logtime_7,
         g.login_cnt_30,
         0 as consumption_7,
         /*i.lev,
         i.roleid,
  ...
  on a.userid = e.userid */
    ...
    left join (select userid,
                 ...
                 from BITASK.t_dw_wm_account_stat_day
  ...
  /*  left join (select userid, sum(cash_need)  as consumption_7
   from bitask.t_dw_wm_glog_shoptrade
  where  logtime >= trunc(sysdate) - 7
  group by userid) h
  ...
  /*  left join (select userid,
               ...
               from bitask.t_dw_wm_gdb_chardata
              where userid > 33
                and logtime = to_char(trunc(sysdate) - 2)) i
  ...
  left join (select groupname,
               ...
               from bitask.t_dw_wm_dic_serverlist) j
  ...
  left join (select c.userid,
               ...
               from (select a.userid, b.f_city_name, count(*) as cnt
                       from (select userid, ip_num
                               from bitask.t_dw_wm_glog_accountlogout
                              where logtime >= to_char£¨trunc(sysdate) - 30£©) a,
   ...
   and k.rn = 1  */
    ...
    left join bitask.t_dw_wm_account_status l
