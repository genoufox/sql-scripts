...
select b.game_ab,
  ...
  left join (select userid,sum(money) / 100 as pay_lastm
               from bitask.t_dw_au_billlog
              where userid > 33
  ...
  left join (select userid, count(logtime) as login_cnt_7,sum(onlinetime)/3600 as login_time_7
 ...
 left join (select userid, sum(yuanbao)  as consumption_7
               from bitask.t_dw_sdyx_glog_shoptrade
              where  logtime >= trunc(sysdate) - 7
              group by userid) i
  ...
  left join (select userid,
               ...
               from bitask.t_dw_sdyx_gdb_chardata
              where userid > 33
  ...
  left join (select groupname,
               ...
               from bitask.t_dw_sdyx_dic_serverlist) k
  ...
  left join (select c.userid,
               ...
               from (select a.userid, b.f_city_name, count(*) as cnt
                       from (select userid, ip_num
                               from bitask.t_dw_sdyx_glog_accountlogout
                              where logtime >= to_char(trunc(sysdate -15,'MM')- 1)
                              and   logtime < to_char(trunc(sysdate,'MM'))
                              ) a,
  ...
  left join bitask.t_dw_sdyx_account_status m
    on a.userid = m.userid
    
 where b.game_ab = 'sdyx'
