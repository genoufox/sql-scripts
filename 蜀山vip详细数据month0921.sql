select 'SH' 项目,
       ...
       k.alias 服务器名称,
       l.f_city_name 登陆城市,
       TRUNC(m.first_logout_time) 最早登陆,
       TRUNC(m.last_logout_time) 最后登录,
  ...
  left join (select userid, sum(money) / 100 as pay_lm
               from bitask.t_dw_au_billlog
              where userid > 33
                and logtime >= trunc(sysdate - 29, 'MM')
  ...
  left join (select userid_,
                    count(distinct trunc(logtime)) as login_cnt_7,
  ...
  left join (select userid_, count(distinct trunc(logtime)) as login_cnt_lm
               from bitask.T_000073_LOG_ROLELOGOUT
              where logtime >= trunc(sysdate - 29, 'MM')
                and logtime < trunc(sysdate, 'MM')
              group by userid_) h
  ...
  left join (select groupname,
   ...
   left join (select c.userid_,
                    c.f_city_name,
                    c.cnt,
                    row_number() over(partition by c.userid_ order by c.cnt desc) as rn
               from (select a.userid_, b.f_city_name, count(*) as cnt
                       from (select userid_, ip2int（peer_） as ip_num 
                      ...
                      group by a.userid_, f_city_name) c) l
    on a.userid = l.userid_
   and l.rn = 1 

  left join bitask.T_000073_account_status M
