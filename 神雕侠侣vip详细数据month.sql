select 'sdxl' 项目,
  ...
  left join (select userid,
               ...
               from BITASK.t_dw_sdxl_account_stat_day
  ...
  left join (select userid, sum(cash_need1) as consumption_7
               from bitask.t_dw_sdxl_glog_shoptrade
              where logtime >= trunc(sysdate) - 7
              group by userid) h
    on a.userid = h.userid
  ...
  left join (select groupname,
               ...
               from bitask.t_dw_sdxl_dic_serverlist) k
  ...
  left join bitask.t_dw_sdxl_account_status m
