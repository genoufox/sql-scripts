select 'xa' 项目,
  ...
  left join (select userid, sum(cash_used) as consumption_7
  ...
  left join (select groupname,
               ...
               from bitask.t_dw_xa_dic_serverlist) k
  ...
  left join (select c.userid,
               ...
               from (select a.userid, b.f_city_name, count(*) as cnt
                       from (select userid, ip_num
                               from bitask.t_dw_xa_glog_accountlogout
                              where logtime >= trunc(sysdate - 15, 'MM')
  ...
  left join bitask.t_dw_xa_account_status m
